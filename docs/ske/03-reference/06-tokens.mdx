...
- A registry token for pulling container images from `ghcr.io/syntasso` (beings with `ghp_`)
- A GitHub token for accessing private release artifacts (begins with `github_pat_`)
