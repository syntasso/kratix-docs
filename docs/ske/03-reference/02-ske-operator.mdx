...
```yaml
  ...
  spec:
    ...
    featureFlags:
      dryRun: false # preview feature, see Kratix Config for details
    tlsConfig:
      certManager:
    ...
    deploymentConfig:
      resources: # Applied as written; leave cpu out of limits to run without a CPU limit
        limits:
          memory: 500Mi
          cpu: 300m
        requests:
...
`deploymentConfig.resources` is applied to the Kratix deployment exactly as written. A `limits` or `requests` block you set replaces the one in the SKE distribution, so leaving `cpu` out of `limits` removes the CPU limit, and an empty block (`limits: {}`) removes limits entirely. If `resources` is not set, the distribution's values are kept. The same applies to `platformManagerDeploymentConfig` and to each `SKEIntegration`.

Kubernetes still requires a request to be no higher than its limit, and copies a limit into the request when the request is not set. Setting neither a CPU request nor a CPU limit is allowed, but the operator will warn: the pod is scheduled with the minimum CPU weight and gets very little CPU when the node is busy.
...
- **`spec.deploymentConfig`** tunes the installed controller's resource requests/limits, applied as written in the same way as for the Kratix deployment.
- The rest of `spec` is type-specific, see [Backstage](../integrations/backstage/installation/generating-backstage-components#installing-the-backstage-controller),
  [Cortex](../integrations/cortex/configuring-cortex#install-the-cortex-integration), or [Portal Controller](../integrations/portal-controller/installation) for the fields each
  type adds.
