...
- A registry token for pulling container images from `ghcr.io/syntasso`
- A GitHub token for accessing private release artifacts

## Syntasso registry token

Use this token to authenticate as `syntasso-pkg` when pulling images from the private Syntasso registry (`ghcr.io/syntasso`).
