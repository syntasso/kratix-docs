...
In this guide we will write a Postgres Promise using the kratix CLI and the SKE Testing Framework.
...
Inside the directory `input/` create a file `object.yaml` containing:
```yaml
...
Earlier in the Promise API, we defined the property `size`, but we have failed to use the input in the script `pipeline.py`.
