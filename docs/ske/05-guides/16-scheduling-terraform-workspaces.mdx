...
The example Promise in this guide provides S3 buckets as a service. Let's go
...
Make sure that, when configuring the settings, you click in "Advanced Options"
and select the subdirectory matching the Destination `spec.path`. For example,
for the Mobile team workspace, we defined the `spec.path` as `mobile-workspace`,
so we will select the `mobile-workspace` subdirectory. Also make sure to check
...
For that, we will use the S3 Bucket example Promise available [here](https://github.com/syntasso/kratix-examples/tree/main/promise-terraform-aws-buckets). You can install the Promise with the following command:

```shell-session
kubectl apply -f https://raw.githubusercontent.com/syntasso/kratix-examples/refs/heads/main/promise-terraform-aws-buckets/promise.yaml
```
...
```shell-session
$ kubectl explain s3.spec.team --api-version=example.syntasso.io/v1alpha1
...
Which brings us back to the diagram at the beginning of this guide, where you can see how SKE will schedule the request to the correct Terraform workspace:
