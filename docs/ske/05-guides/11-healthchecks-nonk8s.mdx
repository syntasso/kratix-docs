...
sidebar_label: Health Checks for non Kubernetes Resource
---

## Pre-requisites

You will need:
1. an installation of SKE for this section. Go to [Configuring SKE](../kratix/configuring-ske/intro) and follow the appropriate guide if you haven't done so already.
2. a configured and deployed Health Agent. You can do that by following the guide on [Configuring the Kubernetes Health Agent](../kratix/ske-health-agent).

It's also recommended to first read through the guide on [how to implement resource health checks](healthchecks) for a more detailed and step-by-step guide on how health checks work in SKE.
...
While the second container `rds-health-check` can schedule a Health Definition that checks for RDS status.
