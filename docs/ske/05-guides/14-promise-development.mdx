...
sidebar_label: Testing Promises
---
...
2. [kratix CLI](/main/kratix-cli/intro) and the [SKE Testing Framework Plugin](/ske/promise-testing-framework/intro).
...
```bash
curl -o dependencies/operator.yaml --silent https://raw.githubusercontent.com/syntasso/promise-postgresql/refs/heads/main/workflows/configure-pipeline/resources/dependencies/operator.yaml
curl -o dependencies/operatorconfiguration.crd.yaml --silent https://raw.githubusercontent.com/syntasso/promise-postgresql/refs/heads/main/workflows/configure-pipeline/resources/dependencies/operatorconfiguration.crd.yaml
curl -o dependencies/postgresql.crd.yaml --silent https://raw.githubusercontent.com/syntasso/promise-postgresql/refs/heads/main/workflows/configure-pipeline/resources/dependencies/postgresql.crd.yaml
curl -o dependencies/postgresteam.crd.yaml --silent https://raw.githubusercontent.com/syntasso/promise-postgresql/refs/heads/main/workflows/configure-pipeline/resources/dependencies/postgresteam.crd.yaml
```
...
Earlier in the Promise API, we defined the property `size`, but we failed to use the input in the script `pipeline.py`.
...
We should first build and push the Resource Configure Workflow so its image can be pulled into your Kubernetes cluster.
...
You can target the Destination to see the StatefulSet created for your Postgres
database with the command:

```bash
kubectl --context=<destination-cluster-config> get statefulset -A
```
...
default     grape   2/2     96s
```

## 🎉 Congratulations

✅&nbsp;&nbsp; That's it! You've successfully created and tested your Database Promise.<br />
