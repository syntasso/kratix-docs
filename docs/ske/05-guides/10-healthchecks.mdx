...
The diagram shows the GitOps path this tutorial uses, and simplifies the last leg: the
`HealthRecord` reaching the Platform's HealthCheck Monitor still goes through a GitOps agent
on the Platform, which is not drawn. If your State Store is a bucket, the SKE Platform
Manager can read the records itself and that agent is not involved at all. See
