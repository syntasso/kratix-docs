...
In this tutorial, you will define a Resource Health Check for the Redis Promise available in the Kratix Marketplace. Let's start by defining what the HealthDefinition will look like.

The [HealthDefinition](/main/reference/health-checks/healthdefinition) object has the following specification:
...
Let's start by defining a very basic Health Definition for the Redis Promise:
...
At this stage, Kratix should execute the Resource Configure Workflow as expected in the Platform cluster, and you should see a new Health Definition object in your State Store. Once the Workflow finishes, you should see a CronJob scheduled in your Destination (it may take a couple of minutes for the CronJob to appear):
...
The CronJob will execute the Workflow defined in the HealthDefinition. You can check the pods:

```shell-session
$ kubectl --context $WORKER get pods
NAME                                                     READY   STATUS      RESTARTS   AGE
healthcheck-redis-default-example-78709-28947855-p5kxt   0/1     Completed   0          2m39s
healthcheck-redis-default-example-78709-28947856-srh9r   0/1     Completed   0          99s
healthcheck-redis-default-example-78709-28947857-8t6b5   0/1     Completed   0          39s
```

You may see one or more pods with `Completed` status. In fact, if you refer back to our HealthDefinition, you will see that we are scheduling the Health check to run every minute (see the `schedule` field).
...
$ kubectl --context $WORKER get pods
NAME                                                     READY   STATUS      RESTARTS   AGE
healthcheck-redis-default-example-78709-28946510-5hr5c   0/1     Completed   0          1s
```
