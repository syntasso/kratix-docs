...
You will need an installation of SKE for this section. Go to [Configuring SKE](/ske/installing-ske/intro) and follow the appropriate guide if you haven't done so already.

You will also need a configured and deployed Health Agent. You can do that by following the guide on [Configuring the Kubernetes Health Agent](/ske/installing-ske/ske-health-agent).
...
The check runs on the Destination and its result is written to a State Store. Getting that result from the State Store onto the Platform can be done by either of two components that we're calling the **HealthCheck Monitor**, and you choose which:

* A **GitOps agent** on the Platform cluster, listening to the State Store and applying the `HealthRecord`s it finds. This works whether the State Store is Git or a bucket, and it is what this tutorial uses.
* The **SKE Platform Manager**, reading the records from the State Store itself, with no GitOps agent involved. This requires the State Store to be a bucket. See [Move health records onto a bucket](/ske/guides/health-records-from-a-bucket).

Whichever HealthCheck Monitor you choose, everything after it is the same. Once the `HealthRecord` is on the Platform, Kratix processes it and updates the resource `status`, and that does not vary with your choice.
...
The diagram shows the GitOps agent option, and predates the naming above, so read two
things with care. The Platform's own GitOps agent is not drawn on the return leg, and the
box labelled `HealthCheck Monitor` is the Kratix step that updates the resource, step 6,
rather than the HealthCheck Monitor that delivers the record in step 5.
...
5. A HealthCheck Monitor then gets the `HealthRecord` onto the Platform cluster: either a GitOps agent on the Platform listening to the State Store, as in this tutorial, or the SKE Platform Manager reading the records from a bucket directly. See [Move health records onto a bucket](/ske/guides/health-records-from-a-bucket) for the second option and how to migrate to it.
6. Kratix will then process the `HealthRecord` and update the resource `status` with the results. This step is the same whichever HealthCheck Monitor delivered the record in step 5.
...
The [HealthDefinition](/ske/reference/healthdefinition) object has the following specification:
...
At this stage, Kratix should execute the Resource Configure Workflow as expected in the Platform cluster. You can watch the pipeline pod run to completion (press `Ctrl+C` to stop watching):
...
If you inspect the State Store, you should see a new Health Definition file there. On the Destination, you can watch the new HealthDefinition object getting applied:

```shell-session
$ kubectl --context $WORKER get healthdefinitions --watch
NAME                    AGE
redis-example-default   10s
```

The SKE Health Agent running in the Destination will react to the new HealthDefinition and schedule a CronJob to run the Health Check Workflow. You can watch for the CronJob to appear (it may take a couple of minutes):

```shell-session
$ kubectl --context $WORKER get cronjobs --watch
...
The CronJob will eventually execute the Workflow defined in the HealthDefinition (you may need to wait a minute or two for it to appear). Watch for the pod running the Health Check Workflow:
...
Since the workflow completed successfully, the SKE Health Agent running in your Destination should have written a `HealthRecord` to the State Store. In this tutorial you have a GitOps agent listening to that State Store, so the Health Record will be applied back to the Platform cluster. (If your State Store is a bucket, the SKE Platform Manager can read it directly instead: see [Move health records onto a bucket](/ske/guides/health-records-from-a-bucket).)
