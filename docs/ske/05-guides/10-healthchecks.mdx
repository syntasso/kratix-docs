...
The check runs on the Destination and its result is written to a State Store. Getting that result from the State Store onto the Platform can be done by either of two components that we're calling the **HealthCheck Monitor**:
