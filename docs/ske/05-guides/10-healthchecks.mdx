...
The check runs on the Destination and its result is written to a State Store. Getting that result from the State Store onto the Platform can be done by either of two things, and you choose which:
...
Whichever you use, what happens afterwards is the same: the Health Check monitor picks up the `HealthRecord` and updates the resource. The monitor is a separate component from both, and it does not change with your choice.
...
The diagram shows the GitOps option, and simplifies it: the `HealthRecord` reaching the
Platform's HealthCheck Monitor goes through the Platform's GitOps agent, which is not drawn.
...
5. The `HealthRecord` is then applied back to the Platform cluster. There are two ways this can happen: a GitOps agent on the Platform listening to the State Store, as in this tutorial, or the SKE Platform Manager reading the records from a bucket directly. See [Move health records onto a bucket](/ske/guides/health-records-from-a-bucket) for the second option and how to migrate to it.
6. The Health Check monitor within SKE will then process the `HealthRecord` and update the resource `status` with the results. This step is the same whichever of the two options in step 5 delivered the record.
