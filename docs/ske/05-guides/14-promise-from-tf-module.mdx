...
1. A Terraform workspace configured configured [watch a Git
...
As the Promise will will generating Terraform configurations, the generated files must be scheduled to a destination that can handle these
...
To ensure it can be used within the promise, build the `kratix-workshop/s3-promise-pipeline` image and push it. If you are using kind, you can load it to your cluster.
