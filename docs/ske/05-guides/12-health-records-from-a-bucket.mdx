...
Reading from an S3-compatible bucket needs nothing new on the Destination: the SKE Health
Agent has written to bucket State Stores for a long time, and the records it writes are
unchanged. Azure Blob Storage is the exception, and needs SKE Health Agent **v0.14.0** or
above, which is the release that can write to it.
...
- An S3-compatible bucket, or an Azure Blob Storage container, that the Destination can
  write to and the Platform can read. **It has to be a bucket**, for the reason below. For
