...
By default, health records travel from a Destination to the Platform through a State Store
that a GitOps agent on the Platform syncs. The SKE Platform Manager can instead read those
records from a bucket itself, which removes the GitOps agent from the health path entirely.
...
```
Today, through a GitOps sync:

  health check ──> health agent ──> bucket or Git ──> GitOps agent ──> HealthRecord
   (destination)   (destination)                       (platform)       (platform)

After this guide, read by the Platform Manager:

  health check ──> health agent ──> bucket ──────────────────────────> HealthRecord
   (destination)   (destination)                  SKE Platform         (platform)
                                                  Manager polls
```
...
:::warning You are giving up a history you may not know you rely on

That commit per check per resource is noise, but it is also a record of how every check
behaved over time. **The bucket path keeps only current state**: one object per resource,
overwritten in place, with no history of earlier runs.

Two consequences worth weighing before you start:

- Commits already in your Git repository stay there. Removing the sync in Step 4 does not
  rewrite history. But the repository stops gaining new health commits as soon as you
  repoint the agent in Step 1.
- Nothing in this path replaces that history. If you need to know how a resource's health
  behaved over time, or whether a check is flapping, publish Platform and Destination
  **metrics to a time-series database**. That is the supported answer, and it is a separate
  piece of work from this migration.

If anything today reads health out of Git commits, such as an audit process or a dashboard,
find it before Step 1 rather than after Step 4.
...
- An S3-compatible bucket, or an Azure Blob Storage container, that the Destination can
  write to and the Platform can read. **It has to be a bucket**, for the reason below.
- **An SKE new enough to have the reader.** There is nothing separate to install and no
  extra RBAC to grant: the component that reads the bucket is a controller inside the SKE
  Platform Manager, and the `HealthSource` CRD and its permissions ship with it. But an
  older SKE will not have it, so check before you plan anything:

  ```bash
  kubectl get crd healthsources.platform.syntasso.io
  ```

  If that returns `NotFound`, upgrade SKE first. Nothing else in this guide will work
  without it, and Step 2 is where you would find out.
...
The health *agent* still supports both, so this is not a limitation you hit on the
Destination. It only applies to the Platform's side of the handover.
...
| Side | Needs | Why |
| --- | --- | --- |
| The **Destination** (health agent) | **write** to the bucket | It puts a record file per resource, and deletes it when the resource goes. |
| The **Platform** (SKE Platform Manager) | **read** the bucket | It lists the prefix and gets objects whose revision has changed. It never writes to the bucket. |
...
Instead, create a **second** set of read-only keys and reference those from the
`HealthSource`. A `HealthSource` deliberately carries its own credential rather than
referencing a `BucketStateStore`, so that one Destination's credential cannot reach another
...
prefix, one read-only credential. Create one per Destination.
...
<Tabs className="boxedTabs" groupId="bucketProvider">
  ...
  metadata:
    ...
    namespace: default
  stringData:
    accountKey: # a read-only storage-account key
  ---
  apiVersion: platform.syntasso.io/v1alpha1
  kind: HealthSource
  metadata:
...
<Tabs className="boxedTabs" groupId="gitopsAgent">
  ...
  ```bash
  kubectl delete application health-records -n argocd
  ```

  With automated pruning enabled, ArgoCD deletes the `HealthRecord`s it created as it goes.
...
If the old records outlive their sync (because pruning was off, for instance), you can
delete them directly. They are the ones **without** the source label:
...
fleet-wide switch: each Destination needs its own `HealthSource` and its own read-only
credential, which is the same property that stops one Destination's credential reaching
another's records.
...
1. **Take one Destination all the way through first**, and prefer a non-production one. The
   ...
   from the Platform, whether the credential really is read-only, whether `spec.path` matches
...
| `secretRef` | object | `name` and `namespace` of the Secret holding the credentials. Required for every `authMethod` except `IAM`, which ignores it. Keys are `accessKeyID` and `secretAccessKey` for `accessKey`, or `accountKey` for `sharedKey`. Needs only list and get on `spec.path`. |
