...
- **You are using ArgoCD, which cannot reconcile from a bucket** (and [may never be able
  ...
  agent) alongside it just for the `health/` path. Reading the bucket in the
  Platform Manager removes that.
- **You are want to use Azure Blob Storage for Health Records**, likely the case
  when you run most of your infrastructure on Azure.
