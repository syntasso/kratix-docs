...
Up until SKE version `<ADD_VERSION_HERE>` and SKE Health Agent version `<ADD_VERSION_HERE>`, the
only way to get health records onto the Platform was a GitOps agent reading them from the
State Store. Starting with SKE version `<ADD_VERSION_HERE>` and SKE Health Agent version
`<ADD_VERSION_HERE>`, the SKE Platform Manager can read the records from a bucket itself, which
removes the GitOps agent from the health path entirely.
...
- **An SKE new enough (version `<ADD_VERSION_HERE>` or above) to have the reader.** There is
  nothing separate to install and no extra RBAC to grant: the SKE Platform Manager reads the
  bucket, and the `HealthSource` CRD and its permissions ship with it. An older SKE will not
  have it, so check before you plan anything:
