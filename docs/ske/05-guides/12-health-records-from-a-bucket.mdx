...
<Tabs className="boxedTabs" groupId="bucketProvider">
  ...
  data:
    ...
    insecure: "true" # only for a store that speaks plain HTTP; omit otherwise
  ---
  apiVersion: v1
  kind: Secret
  metadata:
...
One loose end is cosmetic: the frozen record **files** are still sitting in the old State
Store. Nothing reads them any more, and the agent that wrote them no longer manages them, so
they stay there until you delete them — on Git, one last commit removing the `health/`
directory closes the book.
