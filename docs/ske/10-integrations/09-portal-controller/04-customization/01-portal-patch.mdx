...
For the common cases, setting an owner, adding tags, introducing a resource on its request form,
removing a form field, adding a step beside an existing one, reshaping a Cortex workflow, a recipe
is shorter than the equivalent script, is order-independent, and fails loudly instead of silently
producing the wrong document. For anything it cannot express, write your own container as
described in [Customization](../customization).
