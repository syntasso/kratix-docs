...
A single page is already the default, so this recipe only has an effect on a portal where
[`--single-spec-page=false`](../installation#portal-wide-generate-flags) turned it off, and you
want particular Promises on one page anyway.
