...
Cortex portals are where this matters. On a sync Job, `writer` is the container that calls the
Cortex API:
...
The setup Job that runs when you first connect a Cortex portal calls the Cortex API too. Its
container is called `setup`, and it is that Job's only container, so it goes under `containers`
rather than `initContainers`.

Your variables are added to the ones the controller already sets on that container, which are left
alone.

### Setting a variable on every container

To reach everything the controller runs, name all of the containers. A single document is merged
