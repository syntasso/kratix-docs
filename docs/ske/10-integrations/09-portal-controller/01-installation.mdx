...
:::info Match the list a container is in
A container the controller builds in `initContainers` has to be patched under `initContainers`, and
one it builds in `containers` under `containers`. An entry in the wrong list names a container that
does not exist there, and is dropped without an error. The table above says which is which.
:::
...
Naming a container also lets you add environment variables to it. Use this to pass cluster-wide
settings to the containers that call out of the cluster, such as the egress proxy your network
requires, or `SSL_CERT_FILE` for a private certificate authority.

`generate` and `writer` are the containers that talk to your portal's API on a sync Job, and `setup`
is the one that does so on a setup Job:
...
On a Cortex portal, add a `writer` entry beside `generate` with the same variables.

Your variables are added to the ones the controller already sets on that container, which are left
alone. Set `NO_PROXY` alongside any proxy variable, so the containers that only talk to the
Kubernetes API keep reaching it directly.

One document is merged onto every Job, and an entry for a container that a Job does not run is
ignored. That is what lets the same file cover both sync Jobs and setup Jobs.
