...
|---|---|---|
| SKE Operator | `TODO` | Installing the `portal-controller` integration type |
| Portal Controller | `v0.2.0` | This integration |
| Portal Patch | `0.7.0` | [Declarative customization](./portal-patch/intro), optional |
| `@syntasso/plugin-ske-backend` | `TODO` | Backstage portals |
| `@syntasso/plugin-ske-frontend` | `TODO` | Backstage portals |
| Backstage | `TODO` | Backstage portals, new backend system required |
...
| `adapterConfig` | no, unless bring-your-own | Adapter image, pull secrets and generate flags. Required for a portal type the controller has no built-in adapter for. |
...
Controller's own image, at the version set on `spec.version`. To pin a different adapter image for
one portal, set `adapterConfig.image` to a single image reference (`repo:tag` or `repo@sha256`,
...
For a bring-your-own portal type, any `type` the controller has no built-in adapter for,
`adapterConfig.image` is **required**. There is no bundled default to fall back to, so the
`SKEIntegration` is rejected at admission if it is missing.

### Other `adapterConfig` fields

| Field | Applies to | Meaning |
|---|---|---|
| `image` | all types | Adapter image override |
| `imagePullSecrets` | all types | Extra pull secret names, always merged with `syntasso-registry` |
| `generate.args` | all types | Flags passed verbatim to the generate stage |
| `generate.env` | bring-your-own only | Literal environment variables for the generate stage |
| `writer.mode` | bring-your-own only | `adapter` or `platform` |
| `writer.runsOnDelete` | bring-your-own only | Whether the writer runs on delete |
| `exporter.enabled` | bring-your-own only | Whether to run a status exporter |
| `exporter.deploymentSpec` | portals with an exporter | Overlay merged into the exporter Deployment |

Built-in portal types use their own behaviour for the fields marked bring-your-own, so setting
them on a `backstage` or `cortex` portal has no effect.
...
| Any `adapterConfig.image` | Bring-your-own adapters |
