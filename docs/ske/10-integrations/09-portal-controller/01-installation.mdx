...
[SKE Operator](../../installing-ske/install-with-operator) from a single
...
|---|---|---|
| `--single-spec-page` | `backstage` | Whether the generated Template's `spec` fields share one form page. On by default; set `--single-spec-page=false` for a page per top-level `spec` property |
...
[Portal Patch](./portal-patch/intro) offers per-Promise equivalents through the `singleSpecPage`
and `hideNamespaceManifest` recipes. Use a flag when you want the behaviour everywhere on a
portal, and a recipe when you want it for particular Promises.
...
Those Jobs run in your cluster, so they need to comply with your cluster's policies, and you may
need to change the containers inside them. You configure both in the
`ske-portal-workloads-config` ConfigMap, described in
[Job customization](./customization/job-customization).
