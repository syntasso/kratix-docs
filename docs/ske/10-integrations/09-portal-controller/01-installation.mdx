...
Those Jobs run in your cluster, so they have to live by its rules. If you have an admission policy
that requires certain labels on every workload, it refuses these Jobs and the portal integration
never runs. You may also want them on particular nodes, inside a resource quota, or cleaned up on
your usual schedule.

You control all of that from a ConfigMap called `ske-portal-workloads-config`, in
`kratix-platform-system`, under a `workloads.yaml` key:
...
edit `workloads.yaml`, the Jobs it affects are recreated with the new metadata. Delete the ConfigMap
and they go back to carrying only the controller's own labels.
:::
...
| Which containers exist | You can change a container it builds, but not add one. An added container would run with the pipeline's Kubernetes permissions |
