...
Use this example when you want to change the name of a Cortex workflow. The `generate` step provides a default name for each workflow, describing what the workflow does:

| Generated file         | Default name           |
| ---------------------- | ---------------------- |
| `workflow-create.yaml` | `Create kafka request` |
| `workflow-update.yaml` | `Update kafka request` |
| `workflow-delete.yaml` | `Delete kafka request` |

The example below customizes the workflow name of the Create workflow.

```yaml
apiVersion: platform.syntasso.io/v1alpha1
kind: PortalCustomization
metadata:
  name: kafka-workflow-name
spec:
  portalType: cortex
  promise:
    configure:
        ...
        - name: set-workflow-name
          ...
          args:
            - |
              ...
              patch:
                name: Create Production Kafka Cluster
```
...
| Generated file         | Default message   |
| ---------------------- | ----------------- |
| `workflow-create.yaml` | `Request created` |
| `workflow-update.yaml` | `Request updated` |
| `workflow-delete.yaml` | `Request deleted` |
...
Bind the relevant Promise to `kafka-run-response-message`. See [Customization](../customization#binding-a-promise-to-a-portalcustomization) for the binding label.

## Advanced use cases

A customize container can reshape a Workflow as far as Cortex allows, not just adjust what `generate` writes. For example, split the generated form into several [User Input blocks](https://docs.cortex.io/streamline/workflows/blocks) so users fill in one page at a time. A Branch block can then decide which page comes next, based on their answers. For changes this large, writing the whole workflow file in your customize container is usually simpler than patching the generated one, because little of the original structure survives.
