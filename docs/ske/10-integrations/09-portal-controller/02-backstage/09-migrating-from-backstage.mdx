...
| `singleSpecPage` | Now the default. `singleSpecPage: false` becomes the portal-wide `--single-spec-page=false` flag |
...
One page is now the default, so `singleSpecPage: true` needs nothing carried over. To keep the
old per-property layout, set
[`--single-spec-page=false`](../installation#portal-wide-generate-flags) on the portal, and use
the recipe above for any Promise you still want on one page.
