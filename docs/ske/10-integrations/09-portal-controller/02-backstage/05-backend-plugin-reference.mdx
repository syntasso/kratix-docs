...
See [Configure Backstage](./configure-backstage) for full installation and configuration instructions.
...
steps:
  - id: record-request
    name: Registering your Postgresql request in the catalog
...
both keys rather than sending them empty. The action still comes from the hidden `prAction`
parameter, which the generated Template declares in both modes so an update from the **Manage** tab
is recorded as one:
