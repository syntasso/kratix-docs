...
Every `spec` property shares one form page, titled "Specification". To give each top-level
property a page of its own, turn that off for the portal:
...
portals:
  - name: prod
    ...
    adapterConfig:
      generate:
        args:
          - "--single-spec-page=false"
```

This is a portal-wide setting on the `SKEIntegration`, so every Promise on that portal follows
it. [Field order](#field-order) sets the order fields appear in within the page.
...
<figure className="diagram">
  <img
    className="fullwidth"
    src={FormFieldOrder}
    alt="Instance size, Storage (GB) and Region appearing in that order on the form"
  />
  <figcaption>The first fields appear in the order <code>ui:order</code> names them</figcaption>
</figure>
...
Setting `storageGB` to `null` takes it off the form. The `dependencies` block is keyed on `size`,
the field whose value decides what appears, and gives it one branch per value. The `large` branch
puts `storageGB` back and makes it mandatory, while `small` and `medium` add nothing, so it stays
hidden.
