...
[`PortalCustomization`](../customization) and patch the request template after the Portal Controller's generator has
built it.
...
Setting `storageGB` to `null` removes it from the form. The dependencies block is keyed on `size`, the field whose value determines what appears, with one branch for each value.
