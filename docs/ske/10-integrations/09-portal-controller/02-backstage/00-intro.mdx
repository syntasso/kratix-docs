...
- [Customising the request template](./customising-the-request-template) covers how your Promise
  API becomes the form your developers fill in, and how to change what the controller produced.
- [Migrating from the Backstage Controller](./migrating-from-backstage) covers moving an existing
  integration across.
- [Entity YAML format](./entity-yaml-format) documents the entities the adapter generates.
- [Running the adapter as a pipeline stage](./pipeline-stage) is for platforms that wire the
  generator into each Promise's pipeline by hand rather than letting the controller do it.
