...
- `200` with `metadata.deletionTimestamp`: puts the page into the deleting
  state without storing the returned manifest. The Overview falls back to
  catalog entity annotations, and Resource management controls are unavailable
...
`ScopedEntityPicker` is an `EntityPicker` whose `catalogFilter` may reference
other fields of the same form, so one choice narrows the next. Backstage's own
`EntityPicker` builds its filter from `ui:options` alone, so a picker that
depends on another answer cannot be expressed with it.
...
The properties sit under `spec`, which is why the reference is
`parameters.spec.system` and not `parameters.system`.

`ScopedEntityPicker` accepts every `ui:options` key `EntityPicker` does, such as
...
nothing, the field queries the catalogue **not at all** and renders as a
disabled input, still showing any value it holds. A partially resolved filter
is never issued, because dropping a key would widen the query rather than
narrow it: a filter with every key dropped would return the whole catalogue.
An unclosed `{{` counts as unresolved for the same reason.
...
- A selection is never cleared without evidence. An unresolved filter or a
  failed catalogue read leaves it alone, so opening the Manage tab on an
  existing Resource cannot silently drop a persisted value. A failed read is
  reported through Backstage's error API rather than being swallowed.

The field behaves the same way on the create form and on the [Manage
tab](#resource-manage), since both render through the same scaffolder stepper.

#### Choosing a filter key

Any entity field or relation can be a filter key, and `ScopedEntityPicker`
interprets none of them. Where a key holds a list, matching means "contains".
Keys and values are matched case-insensitively.

Prefer a `relations.*` key over the `spec.*` field it is derived from. Backstage
completes a relation target into a full entity reference, so a Component
declaring `spec.system: payments-system` carries
`relations.partOf: system:default/payments-system` while `spec.system` keeps the
bare name that was typed. An `EntityPicker` always submits a full reference, so
the relation is what it will match. The common relations on a Component are:
...
#### Field ordering on generated Templates

On a Template with per-property parameter pages, the pages are ordered
alphabetically by the Promise's CRD spec key, so a referenced field's key must
sort before the key of the field referencing it: `system` before
`targetService` works, `service` before `system` does not. Both fields may also
sit on the same page, as in the example above, which is what a Template
collapsed into a single specification page gives you.
...
A Template that uses `ui:field: ScopedEntityPicker` in an application where the
field is not registered renders that property as nothing at all, with no error.
