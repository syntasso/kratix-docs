...
- a banner giving the Promise's overall state in one word with the version
  installed and how long it has been able to fulfil requests - the Promise this
  one is part of, when another Promise requires it
...
- **Resource requests**: every request made against the Promise

:::note

**`ske.mode: gitops` cannot supply states.** Live status is only available when
*`ske.mode` is set to `kubernetes` or `hybrid`.
...
- a banner giving the resource's overall status in one word, read from Kratix's
  `Reconciled` condition, or from the health checks once the platform has
  delivered — **Unhealthy** and **Degraded** are the checks' own words, so the
  banner names which kind of unwell rather than only that something is
