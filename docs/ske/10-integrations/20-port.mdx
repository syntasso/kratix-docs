...
enough permissions to manage actions. If required, please refer to the
...
The image can be pulled after replacing VERSION with the target version from the
[releases page](../releases/ske-port-controller):

```bash
docker pull ghcr.io/syntasso/ske-port-controller:<VERSION>
```

If you prefer an to use `imagePullSecret`, you can create the secret with the following
command, replacing YOUR_TOKEN with the token you have been provided with:
...
The `PORT_GITHUB_REPO` must be set up to work with the Port actions.
...
called `port-action.yml` (or what `PORT_GITHUB_WORKFLOW` is set to). The following yaml
will support create, update, and delete flows as well as sharing log data back to Port:

<details>
  <summary>`port-action.yml` contents</summary>
