      ...
      containers:
        - name: set-run-response
          ...
          args:
            - |
              set -e
              yq -i '.runResponseTemplate = "Request has been submitted to Kratix for fulfillment."' /kratix/output/workflow-create.yaml
              yq -i '.runResponseTemplate = "Update has been submitted to Kratix for fulfillment."' /kratix/output/workflow-update.yaml
```
