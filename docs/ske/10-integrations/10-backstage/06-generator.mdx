...
spec:
  ...
  # steps and parameters will be generated according to the API
```

You can customise these values by setting the following `env` variables in the
workflow:

* `TEMPLATE_NAME`: the value of `metadata.name`
...
* `ske.backstage.template/name`: the value of `metadata.name`
