...
Controller. The Backstage Controller continues to work; see [Migrating from the Backstage Controller](/ske/integrations/portal-controller/backstage/migrating-from-backstage)
to move an existing setup across.

## Set up with the Portal Controller

1. Install the Portal Controller with a `backstage` portal, see [Portal Controller Installation](/ske/integrations/portal-controller/backstage/intro).
...
See the [YAML File Format](/ske/integrations/portal-controller/backstage/entity-yaml-format) for examples of these manifests.
...
If you are deploying the Backstage Controller in an air-gapped or network-restricted environment, you must mirror the controller images and release manifests before proceeding. See the [Air-Gapped Installation](../air-gapped-installation/intro#ske-backstage-controller) guide for details.
