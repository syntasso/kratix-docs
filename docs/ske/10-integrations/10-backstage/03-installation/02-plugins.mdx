...
Promise Components must set `kratix.io/type: promise`. Although the route matcher recognises `spec.type: kratix-promise`, the Promise page's data-fetching code reads
`kratix.io/type` to determine the entity type. Without the annotation, the page treats the Promise as a Resource.
