...
The Backstage Controller is installed by the [SKE Operator](../../installing-ske/advanced-install) and can be configured to be deployed when installing the SKE Operator Helm Chart.
...
As you may have already done for SKE Operator image, you can then  the `imageRegistry.host` in your values file:
