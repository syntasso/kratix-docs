...
Plugins for SKE are compatible with Backstage apps created with Backstage version v1.24 or above (tested up to v1.50.4).
...
Starting from Backstage v1.50, **Node.js 22** or higher is required due to native module
dependencies. Make sure your build environment and Docker images use Node 22+.
...
## Installing the plugins in a Backstage App

From the root of the backstage repository, run the following to install the frontend plugin:
...
## Installing the plugins in Spotify Portal for Backstage
...
This will download the plugin as a `.tgz` file on your machine. Create a new directory and unpack this file with:

```
mkdir syntasso-plugin-ske-frontend
tar -xzf syntasso-plugin-ske-frontend-VERSION.tgz -C syntasso-plugin-ske-frontend
```

Edit the `syntasso-plugin-ske-frontend/package/package.json` to add your registry to the `publishConfig`:

```json
{
  "name": "@syntasso/plugin-ske-frontend",
  "version": "0.17.5",
  "publishConfig": {
    # highlight-start
...
Create a `.yarnrc.yml` referencing the `npmAuthToken` key:
...
From the `syntasso-plugin-ske-frontend/package` directory, publish the plugin:

```
yarn npm publish
```

Repeat these steps for the backend plugin - @syntasso/plugin-ske-backend.
...
If your Backstage app still uses the legacy frontend system (Backstage v1.48 or earlier), follow the [Update the EntityPage](#update-the-entitypage) instructions above. Both the legacy and new frontend system exports are included in the plugin, so no code changes are needed when migrating between systems.
