...
with a single, unified integration. They continue to work for at least one more release. See
...
:::note
If you are using the [Backstage Controller](../installation/generating-backstage-components) you will
not need to use `ske-backstage-generator` directly.
:::

`ske-backstage-generator` is an entrypoint built into the `ghcr.io/syntasso/backstage-controller` image. When added as a container in a Kratix Pipeline, it automatically generates the Backstage Template and Component manifests for your Promises and Resource Requests.
...
See [Configuring the Plugins](../installation/plugins) for instructions.

### Backstage Destination {#destination}

You must have a Destination created with the `environment: backstage` label. See the
[Kratix Backstage docs](../installation/creating-entity-state-store#configure-a-destination-for-backstage)
for setup instructions.

## How does it work?

`ske-backstage-generator` is intended to be run as part of your Configure Pipeline for your Promise and Resource Workflows. It will auto-generate Backstage Template and Component manifests, and schedule them to your [Backstage Destination](#destination).
...
## Accessing the Backstage Controller image
...
```bash
docker pull ghcr.io/syntasso/backstage-controller:VERSION
```
...
```bash
# For minikube
minikube image load ghcr.io/syntasso/backstage-controller:VERSION

# For KinD
kind load docker-image ghcr.io/syntasso/backstage-controller:VERSION
```
...
To use `ske-backstage-generator`, update your Promise definition to include it as a container in both the Promise Configure and Resource Configure workflows:
    ...
    promise:
      ...
      - apiVersion: platform.kratix.io/v1alpha1
        ...
        spec:
          ...
          #highlight-start
          - image: ghcr.io/syntasso/backstage-controller:VERSION
...
you should place `ske-backstage-generator` as the last container in the last pipeline of your workflow.

If `ske-backstage-generator` needs to be used more than once in a Workflow, you should utilise the [SKE Backstage
...
This ensures that, when pulling the `backstage-controller` image, Kubernetes will use the
