...
Most platforms should use the [Portal Controller](/ske/integrations/portal-controller/intro) instead of wiring the generator into pipelines by hand: install it with a
`backstage` portal and label your Promise with `kratix.io/portal-backstage`, and it generates the same entities automatically. See
[Generating Backstage Components](../installation/generating-backstage-components). This page documents running the generator manually as a pipeline stage, which you only need
for advanced cases.

:::

The generator is now shipped as the **portal-adapter**, the entrypoint built into the `ghcr.io/syntasso/ske-portal-controller` image (the same binary that used to ship in the
Backstage Controller image). When added as a container in a Kratix Pipeline, it automatically generates the Backstage Template and Component manifests for your Promises and
Resource Requests, driven by the same environment variables and annotations documented below.
...
## Accessing the Portal Controller image
