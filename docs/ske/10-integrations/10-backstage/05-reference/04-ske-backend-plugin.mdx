...
| `ske:record-request` | Records a submitted request so Backstage can show it as pending until the platform has applied it |
...
Records a request that has been submitted but not yet applied to the platform, so Backstage can show it while it is pending. The plugin publishes it as a **pending request** entity in the catalog, and removes it once the request has been applied and the real resource entity appears.

[`pull-request` delivery mode](/ske/integrations/portal-controller/backstage/delivery-modes) depends on this action: the Template writes the manifest to a repository and opens a pull request, and this step records the request so it is visible while it waits. Without it, a submission opens the pull request and then fails. Push mode records nothing, because the resource is created directly and there is nothing to wait for.
...
| `namespace` | `string` (optional) | Namespace of the requested resource. Omit entirely for a cluster-scoped resource |
...
| `pullRequestUrl` | `string` | URL of the pull request carrying the request, read from the output of the step that opened it |
| `requestsRepoUrl` | `string` | Repository the pull request was opened against |
| `manifest` | `string` | The [Resource Request](/main/reference/resources/intro) manifest, verbatim — the same document staged for the pull request, so the record and the pull request cannot describe different requests |

This action produces no output.

**The requester is not an input.** It is taken from the Backstage user who ran the Template, so a Template cannot set or override it. The pending request entity records it as `spec.requestedBy` and is owned by that user, which is what lets a developer find their own requests while they wait. A run with nobody signed in records no requester and falls back to the platform group.

The action authenticates to the plugin as a service rather than as the signed-in user, so it needs no token of its own in the Template.
...
Note the bracketed `steps['pull-request']` rather than dotted access: a step id containing a hyphen cannot be reached with a dot.

## Endpoints

The plugin registers under the `ske` plugin ID, so all endpoints are served under `/api/ske`.

Both endpoints below are generic: they read or delete whatever object matches the `kind`/`apiVersion`/`name`/`namespace` you give them — via the Kubernetes API in `kubernetes` mode, or via git in `gitops`/`hybrid` mode, subject to the backend's configured access (RBAC for Kubernetes, git credentials otherwise). Nothing in the plugin restricts them to [Resource Requests](/main/reference/resources/intro) specifically — you could point them at a `Promise`, or any other CRD or object the backend can reach. In practice, `ske-frontend` only ever calls them for Resource Requests, so that's the intended usage, but it's a convention rather than an enforced scope.
