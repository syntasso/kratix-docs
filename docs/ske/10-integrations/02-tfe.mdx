...
were written to the State Store to fetch the Terraform outputs. If you want to
use a different SHA, you can provide them via the
`/kratix/metadata/ske-tfstate-finder-commit-shas.yaml` file. Populate this file
prior to running the Aspect, and it will use the SHAs for the given Destinations
to fetch the Terraform outputs. This is useful when Kratix is writing to a
branch and the branch is manually squash merged into the main.
