...
By default TFE State Finder uses the SHA of the commit in which the workloads
were written to the State Store to fetch the latest Terraform outputs. If you
want to use a different SHA, you can provide them via the
`/kratix/metadata/ske-tfstate-finder-config.yaml` file. Populate this file prior
to running the Aspect, and it will use the SHAs for the given Destinations to
fetch the Terraform outputs. This is useful when Kratix is writing to a branch
and the branch is manually squash merged into the main. The use case for this is
when you are using SKE to pull request Terraform configurations, instead of
having them applied directly from the branch. In this scenario you need to pass
either the merge commit or squash commit SHA to the Aspect.

If your workflow schedules to a single TFE Destination, you can provide just the
commit SHA:
```yaml
overrides:
...
If your workflow schedules to multiple TFE Destinations, you need to provide a list
