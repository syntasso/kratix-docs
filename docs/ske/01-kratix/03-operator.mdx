...
The Bucket or Git repo needs to mirror exactly the structure of the SKE release
in the Syntasso Enterprise Releases Bucket. In the case of Git, all files
starting with `.` are ignored. The `path` key should point to the directory in
the Bucket or Git repo where the SKE release is stored.
