...
manifests and make them available to the SKE Operator in a location accessible by
the cluster.

### Deploying the SKE Operator

Once you have the images mirrored and the manifests available, you can deploy the SKE
Operator with Helm by providing the necessary configuration:
...
If your private registry requires authentication, you should also provide the
name of the secret containing the credentials:

```bash
--set imageRegistry.imagePullSecret=<SECRET NAME>
```

:::info

The `imagePullSecret` should be created in the `kratix-platform-system`
namespace.
