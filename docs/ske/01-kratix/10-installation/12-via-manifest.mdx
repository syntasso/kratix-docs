...
```gherkin
Acceptance
Given that I have access the the SKE documentation
When I review the SKE Operator Docs
Then I can read about the role of the SKE Operator

Given that I have access the the SKE documentation
When I review the SKE Operator Docs
Then I can read about how to configure the SKE Operator

Given that I have access the the SKE documentation
When I review the SKE Operator Docs
Then I can read about how to provision a Kratix with the SKE Operator
```
...
Definition (CRD) that allows you to manage the Kratix Enterprise installation.
...
To deploy SKE, create a file with the following contents:
...
metadata:
  name: my-kratix
spec:
  version: v0.6.0 # The desired version of SKE
```

Apply the file with `kubectl`:

```bash
kubectl apply --filename <path-to-instance>.yaml
```
