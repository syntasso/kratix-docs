...
Helm can accept values via the command line or a values file. The below examples
show how to use the values file for common use cases.
...
helm install ske-operator syntasso/ske-operator \
  ...
  --create-namespace
  --values FILE_PATH # This is optional. Only use if you are configuring custom values.
```
