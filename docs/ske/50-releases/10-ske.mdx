...
### [0.7.0](http://syntasso-enterprise-releases.s3-website.eu-west-2.amazonaws.com/#ske/v0.7.0/) (UPDATE-ME-ON-RELEASE)
...
* **Breaking Change** - Promise and Resource status condition `PipelineCompleted` is replaced by `ConfigureWorkflowCompleted`
    ...
    * You need to migrate usages of `PipelineCompleted` to `ConfigureWorkflowCompleted` after upgrading.
* Add regular reconciliation for Promise configure workflow and it will rerun at least every 10 hours.
* Add support for configuring [http authorization for Promise Release](../../main/05-reference/11-promises/05-releases.md
