...
description: Documentation for the Hashicord Cloud Platform integration
...
You can now create a Destination, tiying everyting together:
...
generates don't need to be unique. Your terraform code will likely to be more
complex, but this is a simple example to get you started.
...
generate a unique file name. You may need to adapt your code to follow the same
pattern.
...
To use the Aspect, you need to add it to your Promise, as a second workflow in
...
flag is set, the aspect will create a set of `.output` files in
`/kratix/metadata` with the Terraform outputs. You can then use that to create
your owk `/kratix/metadata/status.yaml`.
