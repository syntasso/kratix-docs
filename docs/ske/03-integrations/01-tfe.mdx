...
<details>
<summary>RBAC for Kratix Pipelines</summary>
...
The Aspect will also write a set of `*-output.yaml` files to
...
`<workName>.<destinationName>-<hash>-output.yaml`. This is because each Kratix Pipeline
may schedule to many TFE Destinations, and many Kratix Pipelines may schedule to a single
TFE Destination.
...
```
file=$(find /kratix/metadata -type f -name '*<destinationName>*-output.yaml' | head -n 1)
```
