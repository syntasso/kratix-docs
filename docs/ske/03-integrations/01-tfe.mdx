...
metadata:
  name: tfe
  annotations:
    kratix.io/tfe-credentials: '{"namespace": "default", "name": "tfe-credentials"}'
  labels:
...
* The `metadata.annotations.kratix.io/tfe-credentials` is a JSON object that
...
### Persisting outputs to the Resource Status

By default, the Aspect will add the Terraform outputs from the Resource's scheduled
workloads to the Resource's `status.outputs`. For example, in the
...
In this case, the `*-output.yaml` files in `/kratix/metadata` will still be written, so
