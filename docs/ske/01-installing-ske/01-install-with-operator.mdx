---
description: Install SKE with the SKE Operator, via its Helm chart or its manifests
title: Installing SKE with the Operator
sidebar_label: Installing SKE with the Operator
id: install-with-operator
---
...
This page covers installing SKE with the SKE Operator, via either the Operator's Helm chart
or its manifests directly.

If your cluster is air-gapped, complete [Air-gapped installation](./air-gapped)
first — you'll need your image registry and release manifest mirrors in place
...
To install the SKE Operator and instantiate a new SKE instance, we provide two methods:
...
:::info Fixed installation namespace

**Note** that no matter which installation method you choose, SKE and the SKE Operator must run in the `kratix-platform-system` namespace.
You cannot override this namespace. The Helm command in this guide creates it
with `--create-namespace`. For a manifest-based installation, you must create
the namespace before installing the SKE Operator.

Registry pull secrets, release-storage credentials, and referenced TLS Secrets
must also exist in `kratix-platform-system`.
...
To run SKE, you need a license. You can get one by [requesting a free trial](https://www.syntasso.io/pricing). SKE images are hosted in a private registry, and your cluster needs image pull credentials to access them. By default, the [Syntasso registry token](../reference/tokens#syntasso-registry-token) is used as the credential for the Syntasso registry, but you can also pre-create your own pull secret if you prefer not to inline credentials or if you are pulling from a self-hosted registry.
...
The chart sets requests and limits for every pod it installs. A block you set is applied as written, so to run without a CPU limit remove the `cpu` key from `limits`:
...
or `--set skeDeployment.deploymentConfig.resources.limits.cpu=null`. The same shape applies to `skeOperator.resources`, `skeDeployment.deployJob.resources`, `skeDeployment.platformManagerDeploymentConfig.resources`, and `deploymentConfig.resources` on each integration.

Two things to know when editing these values:

- `null` removes a key; `{}` does not. Helm merges an empty map over the chart's defaults, so `limits: {}` keeps them. Set the keys you want removed to `null`, or write the whole block.
- Setting `resources: null` removes the block from the rendered CR, which means the SKE distribution's own values apply, including its CPU limit. To change the limits, change the keys inside the block.

Kubernetes requires a request to be no higher than its limit, and copies a limit into the request when the request is not set. If you remove both the CPU request and the CPU limit the operator accepts it but warns, because the pod is then scheduled with the minimum CPU weight.
...
Note that for the SKE Deployment (the instance the operator deploys), both webhook and metrics server certificates are required. The SKE Operator itself only requires webhook certificates. If the SKE version you are deploying includes the SKE Platform Manager, certificates for its webhook are also required.
...
skeDeployment:
  ...
  tlsConfig:
    ...
    metricsServerTLSCert: |
      -----BEGIN CERTIFICATE-----
      ...
    # Only required if the SKE version includes the SKE Platform Manager
    platformManagerCACert: |
      -----BEGIN CERTIFICATE-----
      ...
    platformManagerTLSKey: |
      -----BEGIN PRIVATE KEY-----
      ...
    platformManagerTLSCert: |
      -----BEGIN CERTIFICATE-----
      ...
```
...
To avoid inlining certificate data in your values file, reference Kubernetes
Secrets that contain the TLS data. The chart does not create or renew referenced
Secrets.

Before installing the chart, ensure that:

- the `kratix-platform-system` namespace exists
- each referenced Secret exists in that namespace
- each Secret contains the keys `ca.crt`, `tls.crt`, and `tls.key`

The chart cannot reference a Secret in another namespace. Synchronise or copy
any Secret from another namespace into `kratix-platform-system` before
installing the chart.

:::warning

The Secret must exist before you run `helm install` or `helm upgrade`, and the
identity running Helm must be able to read it. The installation will fail if
the Secret is missing or does not contain **all** the required values.

:::

Set `global.skeOperator.tlsConfig.webhookTLSSecretRef` for the SKE Operator.
For the SKE Deployment, both `webhookTLSSecretRef` and
`metricsServerTLSSecretRef` are required. If the SKE version includes the SKE
Platform Manager, `platformManagerTLSSecretRef` is also required:
...
skeDeployment:
  ...
  tlsConfig:
    ...
    platformManagerTLSSecretRef:
      name: "my-platform-manager-tls-secret"
```
...
The TLS certificate for the SKE Platform Manager's webhook server must cover the
following DNS names:

- `ske-platform-webhook-service.kratix-platform-system.svc`
- `ske-platform-webhook-service.kratix-platform-system.svc.cluster.local`
...
data:
  config: |
    ...
    imageRegistry:
      ...
      # Required only for SKE versions earlier than v0.37.0
      pipelineImage: # the image of the separate Pipeline Adapter
```
