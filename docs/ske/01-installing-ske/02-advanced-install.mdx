...
Note that for the SKE Deployment (the instance the operator deploys), both webhook and metrics server certificates are required. As of SKE VERSION-X, SKE includes the Platform
Manager which also requires a platform manager webhook certificate (`platformManagerCertSecretName`). The SKE Operator itself only requires webhook certificates.
...
For the SKE Deployment, both `webhookTLSSecretRef` and `metricsServerTLSSecretRef` are required. If your SKE version includes the Platform Manager (as of SKE VERSION-X), `platformManagerTLSSecretRef` is also required:
...
skeDeployment:
  ...
  tlsConfig:
    ...
    platformManagerTLSSecretRef:
      name: "my-platform-manager-tls-secret" # Required from SKE VERSION-X onwards
```
...
following DNS names (required for SKE versions that include the Platform Manager):
