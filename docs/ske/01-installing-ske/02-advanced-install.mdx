...
For the SKE Deployment, both `webhookTLSSecretRef` and `metricsServerTLSSecretRef` are required. If the SKE version includes the SKE Platform Manager, `platformManagerTLSSecretRef` is also required:
