...
To use Azure Blob Storage, set `provider: azure`. The `endpoint` becomes the
storage-account URL, `bucketName` becomes the container, and the only supported
`authMethod` is `sharedKey`, whose Secret holds a single `accountKey`:
...
:::info You don't need a GitOps agent for buckets

If your state store is a bucket, the SKE Platform Manager can read the records itself, so
there is no GitOps agent to run on the Platform for health checks. See
[Move health records onto a bucket](/ske/guides/health-records-from-a-bucket).
