---
description: Install SKE via a preconfigured Job
title: Preconfigured installation
sidebar_label: Preconfigured installation
---
...
This page covers the preconfigured installation — the fastest way to get SKE running, 
using a Job that installs all the required tools automatically, including cert-manager and Flux. 
It's intended for development and testing on a clean, disposable cluster, not production, and 
assumes cert-manager and Flux aren't already running there.

If that doesn't match your situation — you're targeting production, need to customise the install, 
already have cert-manager, Flux, or another GitOps tool in place — see [Advanced installation](./advanced-install), 
or check the [installation overview](./intro) to confirm which method fits.

<PartialPointToKratix />

This guide walks you through a **preconfigured installation** of
Syntasso Kratix Enterprise (SKE) with a **preconfigured Backstage**. It is the fastest way to get started with SKE. 
It is not intended for production use, but rather to get you started and experimenting
quickly. 

## Prerequisites

To install SKE using this guide, you'll need access to a Kubernetes cluster.

We recommend using a clean, disposable cluster for this preconfigured installation and you can
...
:::caution
The preconfigured installation installs `cert-manager` and `Flux` for you. If either is already
...
Backstage. To highlight this, a preconfigured Backstage instance is included in
the preconfigured installation.

Before installing, create the namespace and secret referencing your [Syntasso registry token](../reference/tokens#syntasso-registry-token) so your cluster can access the private container registry:

:::info Fixed installation namespace

SKE and the SKE Operator must run in the `kratix-platform-system` namespace.
You cannot override this namespace. The Helm installation can create it for
you; for a manifest-based installation, create it before installing the SKE
Operator.
...
# secret is required for all namespaces where resource requests are made
kubectl create secret docker-registry syntasso-registry \
  --namespace=default \
  --docker-server=ghcr.io \
  --docker-username=syntasso-pkg \
...
After a few moments you should see the Backstage UI. Click on "Login as Guest"
and you should see a page with the Postgres Promise Component listed in the
catalog (you may need to refresh the page):
