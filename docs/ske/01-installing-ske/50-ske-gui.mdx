---
title: Installing the SKE GUI
description: How to install and configure the Syntasso Kratix Enterprise GUI as part of your SKE deployment.
...
This page explains how to install the SKE GUI. For documentation on how to use the GUI, please refer to [SKE GUI usage guide](/ske/guides/ske-gui).
...
SKE GUI image is hosted in a private registry. To access the SKE GUI image, you will need to authenticate using the [Syntasso registry token](../reference/tokens#syntasso-registry-token) you have been provided with by Syntasso.
...
To use this secret in the SKE GUI chart, you can customise `imageRegistry.imagePullSecret` in your values file.
...
You must ensure that image `ghcr.io/syntasso/ske-gui` is mirrored to your own image registry and tagged according to the version.
...
The helm chart creates a service account in the `kratix-platform-system` namespace with permission to access the SKE GUI. If you do not wish to create such a service account, or you would like to give the service account a custom name,
