...
The helm chart creates a service account in the "kratix-platform-system" namespace with permission to access SKE GUI by default.
If you do not wish to create such service account, or you would like to give the service account a custom name,
you can customize in the values files:
...
To access the GUI, you could port forward using SKE GUI service, run:

```bash
kubectl -n kratix-platform-system port-forward svc/ske-gui 8080:80
```

Alternatively, if you have created an ingress server with the helm chart. You can also access the GUI through your configured hosts.
