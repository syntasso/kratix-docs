...
the SKE Operator. Beyond full lifecycle support, it provides a simpler interface
...
It needs access to the private registry where the SKE images are distributed
through. Support for installing SKE in **air-gapped environments** instead using
the SKE CLI is coming soon.

In the meantime, you can install [using Helm](#via-helm-chart) and follow the [setup for air-gapped environments](./air-gapped).
