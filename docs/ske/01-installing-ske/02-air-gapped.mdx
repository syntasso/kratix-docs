...
This page covers the mirroring and configuration needed to install SKE in an air-gapped environment
 — setting up your own image registry and release manifest location before pointing the SKE Operator
  ...
  then follow [Installing SKE with the Operator](./install-with-operator), using the registry and manifest locations 
  you've set up here in your `values.yaml` or ConfigMap.

If your cluster isn't air-gapped, you likely don't need this page — check the
[installation overview](./intro) for instructions instead.
...
With the mirrors in place, you can install SKE via the Helm chart. Refer to the [installation guide](/ske/installing-ske/install-with-operator#via-helm-chart) for more details, and make sure to configure the [air-gapped mirrors](/ske/installing-ske/install-with-operator#air-gapped-mirrors).
