...
This page covers the mirroring and configuration needed to install SKE in an air-gapped environment
 — setting up your own image registry and release manifest location before pointing the SKE Operator
  at them, rather than editing the manifests directly. Complete the steps here first, 
  then follow [Advanced installation](./advanced-install), using the registry and manifest locations 
  you've set up here in your `values.yaml` or ConfigMap.

If your cluster isn't air-gapped, you likely don't need this page — check the [installation overview](./intro) 
to confirm which method fits, or head to [Preconfigured installation](./preconfigured-install) for the 
fastest path in development and test.
...
If you want to host your own manifests and/or images you
...
There is more information on the [SKE tokens page](/ske/reference/tokens) about which one to use for registry access.
:::
