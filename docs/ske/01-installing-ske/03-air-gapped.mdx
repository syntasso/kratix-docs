...
```bash
docker login ghcr.io -u syntasso-pkg -p {SYNTASSO_TOKEN}
```

Pull the images at the desired tag

```bash
docker pull ghcr.io/syntasso/ske-platform:latest
```

Tag the image to be pushed to your internal registry

```bash
docker tag ghcr.io/syntasso/ske-platform:latest ${INTERNAL_REGISTRY}/syntasso/ske-platform:latest
```
