...
To run the workflows, Kratix creates a [Service Account](https://kubernetes.io/docs/concepts/security/service-accounts/) on the Resource namespace (or in the `kratix-platform-system` for Promise workflows). This Service Account is used by the Job to run the Workflow. If the `spec.serviceAccount` is set in your Workflow, Kratix does not create the Service Account and uses the one you provide.
...
You can find the Cluster Role and Cluster Role Binding by filtering by them by the `kratix.io/promise-name` label.

#### The Workflow SA RBAC

To execute the Workflow, Kratix also gives the Workflow Service Account the right permissions. For that, it creates Roles and Role Bindings. Those objects are created on the same namespace as the Resource (or in the `kratix-platform-system` for Promise workflows).
...
Once a Job is created, Kubernetes creates an associated [Pod](https://kubernetes.io/docs/concepts/workloads/pods/) to execute the containers specified in the Workflow. If you inspect the Pod, you find the containers defined in the Promise document, as well as a few containers Kratix itself injects. Those are:
...
One of the last containers executed as part of a Workflow is the `work-creator`. This container is responsible for creating the Work associated with the Workflow. A Work is a Kratix Object that represents the output of the Workflow, i.e., it contains the documents the Workflow persists into `/kratix/output` as part of its execution. The Work is created in the same namespace as the Resource (or in the `kratix-platform-system` for Promise workflows).

The Work Object contains a similar set of labels as to the Job.

Once a Work is created, Kratix runs its scheduler. The scheduler looks into the Work Object and determines which [Destination](/main/reference/destinations/intro) can take which bits of the Work, since a single Work can contain multiple documents that can be scheduled to multiple destinations. Once a Destination is found, Kratix creates a WorkPlacement object that associates the Work with the Destination.
