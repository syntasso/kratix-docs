...
reference these CRDs. See [Custom Resource Definitions (CRDs)](/main/platform-concepts/custom-resource-definitions)
for how Promises define their APIs. The CRDs of note that come out of the box with Kratix are:
...
Those request resources are namespaced, so access is typically granted with
namespace-scoped `Role` and `RoleBinding` pairs per team or environment. See
[Namespaces](/main/platform-concepts/namespaces) for how this shapes isolation
