...
:::warning WorkPlacement is part of the source of truth for scheduling

If you restore Kratix without `WorkPlacement`s, the scheduler can make a new
selection and place work on a different Destination. That can lead to
non-deterministic placement and unexpected drift. Ensure your backups include
`WorkPlacement`s and validate they are present after restore.

If you cannot restore `WorkPlacement`s, tighten scheduling rules so each
`Work` resolves to exactly one Destination.
...
Syntasso for a more in depth design discussion.
...
- **Kratix resources**
  - [All Kratix CRDs](/main/platform-concepts/kratix-resources)
  - All Resource Requests for the Promises
...
For more detail, see [Workflow idempotency](/main/reference/resources/workflows#idempotency).
