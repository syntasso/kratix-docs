...
- Consider using circuit breakers with sensible timeouts
  and backoff to avoid repeated failing calls during outages.
...
Tool examples in this page are guidance, not a prescriptive runbook. Your exact
...
Restore sequencing matters. If reconciliation tools resume before cluster state
is fully restored, they may prune or overwrite resources.

Use a tested restore order for your platform:
...
regular DR exercises and update it when tooling or architecture changes.
...
- Validate the recovered destination against your expected state before
  re-enabling dependent automation.
