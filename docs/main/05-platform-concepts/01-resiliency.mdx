...
asynchronously towards the desired state that the document define.
...
Like other cluster state, `Work`/`WorkPlacements`s are persisted to etcd.

This matters for HA/DR because `Work` is replayable from the control plane: after a
restart, Kratix can observe existing `Work` resources and continue convergence.
...
- Declarative output is recorded in-cluster as `Work` resources (persisted to etcd),
  which makes it observable and replayable during recovery.
