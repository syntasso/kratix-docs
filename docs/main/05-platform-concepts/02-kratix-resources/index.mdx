...
slug: /main/platform-concepts/kratix-resources
---

Whenever a new Promise is installed, or a new Resource is requested, Kratix creates a series of Kubernetes Objects to complete the installation or to fulfill the Promise. Request resources are namespaced; see [Multi-tenancy](/main/platform-concepts/multi-tenancy) for how that affects isolation and access.
...
* The `status-writer`: this container uses the `/kratix/metadata/status.yaml` file to update status. For Promise workflows, only `status.message` is updated from this file.
