...
In the [Kratix Marketplace](https://docs.kratix.io/marketplace) there's an example Compound Promise that provides Apps-as-a-Service. Go ahead and install that promise:
...
Kratix resolves each required version against the [Promise
Revisions](../reference/promises/promise-upgrade/promise-revisions) of the required Promise, and Promise Revisions are
retained by Kratix across Promise upgrades. See [Compound
Promises](../reference/promises/compound) for the full resolution rules, for example what happens when a required
Promise stops being Available.
...
The App-as-a-Service Promise `destinationSelectors` are set to target clusters with `matchLabel`
