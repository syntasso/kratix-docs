...
**Who this is for:** Application Developers or CI pipelines that consume Promises.  
These users interact with the CRDs that Promises create but should not modify Kratix internals.

---

### Option A. Full access to all resources within one group

Developers can create and manage any resource type under `services.internal.acme.org`, which includes `Database` and `VM`.
