...
Within the resource `status`, Kratix maintains the `healthStatus` field. In this field, Kratix records the health of the resource as reported by its Health Records; workflows cannot write it from `status.yaml`. The field has the following format:
  ...
  healthStatus:
    ...
    - state: healthy
      ...
      details:
        readyReplicas: 2
      source:
        name: healthrecord-example-record-1
        namespace: default
    - state: healthy
      lastRun: 1531958500
      details:
        readyReplicas: 4
      source:
        name: healthrecord-example-record-2
        namespace: default
```

The `healthStatus` field contains a `state` subfield and a list of `healthRecords`. The `state` subfield indicates the overall health of the resource and is calculated from the state of each of the `healthRecords`. For a versioned Promise it also carries `promiseVersion`, the version whose Configure workflow last produced a Health Definition for the resource; see the [status reference](/main/reference/resources/status#health-checks) for how the state is reset when that version changes.
...
Promise writers can use `/kratix/metadata/status.yaml` to populate the `healthRecord` status field. Alternatively, they can write a HealthRecord CustomResource and schedule it to the Platform cluster in their Pipelines.
...
You can have external systems generating the HealthRecord CustomResource and, via GitOps, continuously updating the health information of the resources in your Platform.
