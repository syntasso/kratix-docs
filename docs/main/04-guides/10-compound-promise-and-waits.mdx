...
Now open the pipeline.sh script in `workflows/resource/configure/create-runtime/create-runtime/scripts/pipeline.sh` and update it to:
...
Now there is only one thing left to do in our Compound Promise:
...
Warning: Required Promise "postgresql" at version "v1.0.0-beta.3" not installed
...
The container image you use for `spec.image` should serve HTTP on port `8000`, and handle the PostgreSQL environment variables only when they are present. 
