...
which is responsible in creating the spefified sub-promise requests:
...
1. Write `/kratix/metadata/status.yaml` with:
     - whether bucket was requested
     - whether PostgreSQL was requested
     - the generated bucket request name
...
Back in the pipeline script, let's update it to include this request when needed.  Add the following code snippet:
...
Let's include a request to the Bucket Promise if the user requests an s3 bucket. It's API looks like this:
...
Similar to the PostgreSQL request, when the user requests an S3 Bucket, we should add the Bucket request to the output directory. Right after the PostgreSQL block you just added, include the following:
...
And lastly we need to write `/kratix/metadata/status.yaml` with:
     - whether bucket was requested
     - whether PostgreSQL was requested
     - the generated bucket request name
     - the generated PostgreSQL request name

After the bucket request section in the script add the following:
...
This pipeline is responsible for reading the status written by `create-dependencies`, and do the following operations
   - If a bucket was requested:
     - verify the bucket resource is ready
     - read the surfaced `s3_bucket_arn` from bucket status
   - If PostgreSQL was requested:
     - verify the PostgreSQL resource is ready
   - If any requested dependency is not ready:
     - write `/kratix/metadata/workflow-control.yaml`
     - include `retryAfter`
     - optionally include `maxAttempts` and `message`
   - If all requested dependencies are ready:
     - write `/kratix/metadata/status.yaml` with the resolved values needed by the Runtime request

Let's implement this, in the `workflows/resource/configure/wait-for-dependencies/wait-for-dependencies/scripts/pipeline.sh` add the following:
...
TBA; the rest of the scripts


#### `create-runtime`:
In the `create-runtime` pipeline we will read the the resolved values from status, and ONLY after our dependencies are available we will create the
`Runtime` request from the Runtime Promise. This pipeline also injects to the application's environment the `BUCKET_NAME` and `BUCKET_ARN` only when a bucket was requested. And in a similar manner, it only injects `PGHOST`, `DBNAME`, `PGUSER`, and `PGPASSWORD` only when PostgreSQL was requested
...
Warning: Required Promise "s3" at version "v0.1.0" not installed
...
kubectl --context $PLATFORM apply --filename s3-promise/promise.yaml # needs to change
...
postgresql   Available   postgresql   marketplace.kratix.io/v1alpha2   v1.0.0-beta.3   Promise configured
runtime      Available   Runtime      marketplace.kratix.io/v1alpha1   v1.0.0          Promise configured
s3           Available   Bucket       example.kratix.io/v1alpha1       v0.1.0          Promise configured
```
...
metadata:
  name: app-stack
spec:
  image: python-app:v0.1.0 # you can use this one, or build your own
  database:
    driver: postgresql
  bucket:
    name: myapp-bucket
...
```sh
docker build -t python-app:v0.1.0 python-app/
kind load docker-image python-app:v0.1.0 -n worker
```