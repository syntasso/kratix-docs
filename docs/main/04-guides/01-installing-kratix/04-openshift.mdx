...
import PartialPlatformSize from '../../../_partials/installation/_platform_size.md';
```
...
Kratix docker images are hosted on the `syntasso-community.docker.reo.dev` registry, ensure you've
...
`syntasso-community.docker.reo.dev` registry even after adding it as a supported registry:
```
...
...
Failed to pull image "syntasso-community.docker.reo.dev/syntasso/kratix-platform": fetching target platform image selected from image index: reading manifest sha256:... in syntasso-community.docker.reo.dev/syntasso/kratix-platform: received unexpected HTTP status: 500 Internal Server Error
```

If this event occurs, we recommend one of the following actions:

1. Use the Docker Hub Registry instead. Update the image references from `syntasso-community.docker.reo.dev/syntasso` to `syntasso`.
