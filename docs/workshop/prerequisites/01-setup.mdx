...
1. An **S3 CLI** for inspecting the state store bucket: <br /> the Kratix repository ships one,
   ...
   bucket (the next page adds it to your `PATH` so the workshops can call `s5cmd` directly). <br /> If you would rather use your own S3 client, `make bucket-env` prints the
