...
For this step, we will use a Python script powered by the [Kratix Python SDK](https://github.com/syntasso/kratix-python). The SDK provides a set of helper functions to make it even easier to write Kratix Promises.

Create a `database-configure` file in the `workflows/resource/configure/mypipeline/kratix-workshop-app-pipeline-image/scripts` directory and make it executable. Next, copy the contents of [this file](https://raw.githubusercontent.com/syntasso/kratix-docs/refs/heads/main/docs/workshop/writing-a-promise/_partials/compound-promise/02-database-configure) into it by running the following:
...
Since the `database-configure` script is written in Python with the SDK, update the Dockerfile to include the Python dependencies.
