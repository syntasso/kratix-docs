---
id: compound-promises
title: "Part 3: Using Compound Promises"
...
👈🏾 Previous: [Installing a Promise](installing-a-promise) <br />
...
## <span id="what-is-a-compound-promise">What's a Compound Promise?</span>
...
## <span id="write-a-compound-promise">Installing a Compound Promise</span>
...
Before jumping in, verify that Kratix is still up and running on your Platform
cluster:
...
cluster itself as an available Destination. That's because the Dependencies for
the Compound Promises are Promises themselves, therefore they need to be
scheduled to the Platform cluster.
...
1. Install and configure Flux
1. Register the cluster as a Destination with Kratix

There's a script in the `kratix` directory that will do exactly that. This
script includes the same [Destination setup](./installing-kratix#destination-setup)
steps you performed previously.
...
Just like when you registered the worker cluster, you should also see a `kratix-worker-system`
namespace, indicating that Flux is correctly configured. You can verify this
with:
...
You have successfully registered the Platform cluster as a Kratix Destination. 

You are now ready to install the compound Promise.
...
The dependencies for the sub-Promises, on the other hand, are installed on the
Worker destination. You can validate that by checking the deployments on the
Worker:
...
across the fleet. If a Destination's labels change, Kratix will automatically
converge on the expected system state.
...
seconds for the `postgresql` and `nginx` pipelines to start.
...
It will take a couple of minutes for the Todo App to start, and it may cycle
through a few states, including _Error_, before it eventually succeeds.
...
✅&nbsp;&nbsp;This tutorial concludes the content on how to operate Kratix.<br />
