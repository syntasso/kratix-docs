...
s coveredp previously, the `spec.api` field is where you define the API for your Promise. You will need to add a new field called `dbDriver` to the `spec.api.openAPIV3Schema` field. Locate the `schema` field in your `promise.yaml` and add the following:
...
Create a `database-configure` file in the `workflows` directory and make it executable. Next, copy the contents of [this file](https://gist.github.com/syntassodev/7cfae7b53bc54615cf351760a8377ba2) into it:
...
Inspect those files. You should see that the `output/platform/postgresql.yaml` file has been created and contains the details of the PostgreSQL service, and the `metadata/destination_selectors.yaml` is telling Kratix to request the PostgreSQL from the platform cluster. The `deployment.yaml` file should also contain the details of the PostgreSQL service in the `env` block of the first `container`.

```yaml title="app-promise/workflows/test/output/deployment.yaml"
env:
- name: PGPASSWORD
  valueFrom:
  secretKeyRef:
...
You should see the both the App and the PostgreSQL getting deployed on the Worker cluster:
...
Amazing! You have successfully created a Compound Promise that can request a PostgreSQL service alongside an App-as-a-Service. You can now go to the [http://todo.local.gd:31338](http://todo.local.gd:31338/) and add some Todos. Then, delete re-run the command to delete the App. Once the App gets redeployed, you should see that the Todos are still there, as the state of the App is now persisted in the PostgreSQL service.
