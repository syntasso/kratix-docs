...
👈🏾&nbsp;&nbsp; Previous: [Updating the Resource status](updating-status)<br />
...
At present, users can self-serve their own Apps-as-a-Service but we know that eventually users will need a way to persist the state of their applications. A database is another offering that can be provided as-a-service to users, providing platform teams with control over where the services are deployed. As, in this case, the need for a database is tied to the functionality of the App-as-a-Service, we can make the Database platform offering something that can requested alongside the App-as-a-service.

To achieve this, we will make it possible for users to request a Postgresql service alongside their App.

### Learn about Compound Promises

Compound Promises are Promises that, in their Dependencies, contain other Promises. That ability allows Platform teams to deliver entire stacks on demand, instead of simple databases or services.
...
- The Compound Promise must instruct Kratix to install its Dependencies (i.e. the other Promises) in the platform cluster

Your platform cluster should already be registered as an available Destination. To validate this, run the following:

```bash
kubectl --context $PLATFORM get destinations
```

You should see both the platform and the worker cluster listed. If not, return to [Installing Kratix](installing-kratix) and follow the instructions for registering the platform as a Destination.

### Configuring a Compound Promise

When the Promise that a given Promise requires has already been defined, it can be specified as such. We are going to make Postgresql available to the App-as-a-Service Promise via an existing Postgresql promise. 
...
```mdx-code-block
import FullPromise from "!!raw-loader!./_partials/07-promise-with-postgres-dependency.yaml"
```
...
It is not only users of the App-as-a-Service who require the `postgresql` and as result a result, it doesn't make sense for the Postgresql promise to be tied to the App-as-a-Service promise. You instead want to deploy the Postgresql Promise on its own and to ensure that the Postgresql Promise is installed when you install or update any Promise to utilise it.
...
This is because the Postgresql promise is no longer being installed by the App-as-a-Service and now _expects_ the Postgresql Promise to exist already. So let's fix that, run the following to install the Postgresql Promise:
...
::info
You might have noticed that the above command uses a Promise Release to install the Postgresql Promise, we will not be exploring this in much depth but you can look at the [documentation](releases) for more information
:::
...
Create a `database-configure` file in the `workflows` directory and copy the contents of [this file](https://gist.github.com/syntassodev/7cfae7b53bc54615cf351760a8377ba2) into it.

```bash
curl -o database-configure --silent https://gist.github.com/syntassodev/7cfae7b53bc54615cf351760a8377ba2
```
...
As the the contents of `/kratix/output/platform` needs to go to the Platform cluster, it explicitly states that the output needs to be directed to the Platform:
...
Use the `test-pipeline` script to the workflows by running the following:

```bash
./scripts/test-pipeline "resource-configure && /database-configure"
```

You should now see the details of the newly provisioned Postgresql in the `env` of your `outputs/deployment.yaml`.

And now for the final steps, updating your `promise.yaml`.

Update your `Dockerfile` to add the new `database-configure` script:

```dockerfile
COPY database-configure database-configure
```

And update your `promise.yaml` to add the `database-configure` step to the the `spec.workflows.resource.configure` list:
...
- what is a compound promise
- update the api
- update pipelines to read api field
- update destinations
