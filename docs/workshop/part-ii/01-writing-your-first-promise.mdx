...
To build the Promise, you can use the [Kratix CLI](/main/kratix-cli/intro). With the CLI, you can quickly initialise a Promise and define its API.
...
```shell-session
kratix update api --property image:string --property service:object --property service.port:int
```
...
The generated Dockerfile uses the `kratix-pipeline-utility` image as a base image. This image provides a set of utilities that make it easier to write Kratix Pipelines. You may also notice that image will execute a script called `pipeline.sh` when the container starts.
...
Make sure to update the Dockerfile to reflect the new script name:

```diff
- ADD scripts/pipeline.sh /usr/bin/pipeline.sh
+ COPY resource-configure /usr/bin/resource-configure

- RUN chmod +x /usr/bin/pipeline.sh
+ RUN chmod +x /usr/bin/resource-configure

- CMD [ "sh", "-c", "pipeline.sh"]
+ CMD [ "sh", "-c", "resource-configure"]
```

Replace the contents of the `resource-configure` scruot with the following:
...
When you ran the `kratix add container` command, the CLI automatically included the `workflow` key into the Promise, with the `resource.configure` Workflow defined. As you will add more scripts to the same image, open the `promise.yaml`, set the `command` key to the script you just created.
...
As part of the Promise `dependencies`, you have included the NGINX Ingress
