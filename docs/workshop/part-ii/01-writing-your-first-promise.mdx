...
👉🏾 Next: [Improve your Promise API](./improving-your-promise-api)
...
Kratix Promises are the encapsulation of platform offerings. They provide a structure to manage the complexities of providing _something-as-a-service_, including the definition of how a user can request the Resource, the steps to provision the requested Resource, and how to provide access to the Resource.
...
In this tutorial, you are going to create a Promise that delivers Apps as a service. It will bundle the Kubernetes resources needed to run an application, including a Deployment, Service, and Ingress.
...
This tutorial will not go through details on how to write a Kubernetes CRD, but check out the [Kubernetes documentation](https://kubernetes.io/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/#create-a-customresourcedefinition) for further details on the topic.
...
You can start by defining an interface for the applications teams to deploy their applications. This interface is the contract of what can be configured and within what parameters.
...
For example, you could allow them to configure all aspects of the underlying Deployment and Ingress rules. That option provides your users with the maximum autonomy and configurability. However, it may be a lot to understand and misconfiguration of services can cause both cost and security issues in the future.

On the other hand, you could go to the other end of the spectrum, hiding all the options and asking users to provide only the absolute necessary pieces of configuration. This allow for less configuration errors, but it also limits the flexibility of the service.
...
```mdx-code-block
import PromiseCRD from "!!raw-loader!./_partials/01-promise-api.yaml"
```

<CodeBlock language="yaml">{PromiseCRD}</CodeBlock>

To keep the promise all in one place, create a directory called `app-promise` and a file `promise.yaml` inside it:
...
```mdx-code-block
import PromiseWithAPI from "!!raw-loader!./_partials/02-promise-with-api.yaml"
```

<CodeBlock language="yaml" title="app-promise/promise.yaml">{PromiseWithAPI}</CodeBlock>
...
As previously mentioned, this Promise will create a Deployment, a Service, and an Ingress. Deployments and Services are builtins in Kubernetes, so you can use them directly. However, to use the NGINX Ingress Controller, you need to have it installed in your worker cluster _prior_ to any user deploying their applications.
...
If you follow the NGINX Ingress Controller [installation instructions](https://kubernetes.github.io/ingress-nginx/deploy/#provider-specific-steps), you will find that you need a the resources defined in [this file](https://gist.githubusercontent.com/kirederik/b39aaefbc1a4ea48d98039ba77eb676c/raw/831758503c86b5accbca3eff416b15e1aaf827e9/nginx-patched.yaml) to be installed in the cluster.
...
If everything goes well, the command above should not output anything. You can verify that the dependencies were added by checking the contents of the `promise.yaml` file. You should see the contents of the `dependencies.yaml` file under the `dependencies` key.
...
Start by creating a new directory called `workflows` inside the `app-promise` directory. Then, inside the directory, create a `Dockerfile`. The Dockerfile will be the container image that will be executed as part of the `resource.configure` Workflow:
...
```mdx-code-block
import Dockerfile from "!!raw-loader!./_partials/03-Dockerfile"
```

<CodeBlock language="dockerfile" title="app-promise/workflows/Dockerfile">{Dockerfile}</CodeBlock>
...
```mdx-code-block
import ExecutePipeline from "!!raw-loader!./_partials/04-execute-pipeline.sh"
```

<CodeBlock language="bash" title="app-promise/workflows/execute-pipeline">{ExecutePipeline}</CodeBlock>
...
You may also notice that the script uses the `/kratix/output` directory to store the resources that must be created. At the end of the workflow, Kratix will deploy the resources stored in the `/kratix/output` directory in one of the available Destinations.
...
With the pipeline image defined, you can now configure the `resource.configure` Workflow. Open the `promise.yaml` file in your editor and add the following content under the `spec` key:

```mdx-code-block
import ResourceWorkflow from "!!raw-loader!./_partials/05-resource-workflow.yaml"
```

<CodeBlock language="yaml" title="app-promise/promise.yaml">{ResourceWorkflow}</CodeBlock>
...
```mdx-code-block
import FullPromise from "!!raw-loader!./_partials/06-full-promise.yaml"
```

<CodeBlock language="yaml" title="app-promise/promise.yaml">{FullPromise}</CodeBlock>
...
You can switch hats now and assume the role of a App developer trying to deploy their application.

## Using the App Promise {#using-the-app-promise}

As a developer, you can check what are the available promises by running the following command:
...
✅&nbsp;&nbsp; You have written your first Kratix Promise.
