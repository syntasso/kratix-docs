...
Inspect the `test/metadata/status.yaml` file.
...
You may also notice that there's a condition called `ConfigureWorkflowCompleted`. This is a special condition that is automatically added to the status of the Resource, and set to `True` when the pipeline completes successfully.
