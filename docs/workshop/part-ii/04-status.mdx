...
With that in-mind, you can update your existing `resource-configure` script to surface some information about the Resource in the `status.yaml` file. One of the things you may want to surface is, for example, the URL of the running application. You also want to expose the date the resource was created, as a human-friendly string.
...
<figure className="diagram">
  <StatusUpdateFlowDiagram className="medium"/>
...
`True` when the Configure workflow completes successfully.
