...
```bash title="app-promise/workflows/create-bucket"
...
Make sure to add the following lines to the `Dockerfile` to include the new files:
...
COPY create-bucket create-bucket
```
...
If you are running the workshop on a Mac, you will need to update the `MINIO_ENDPOINT` to `host.docker.internal:31337`.
...
Great! That proves our pipeline stage works end-to-end. You can now apply your updated Promise to your Platform cluster:

```bash
kubectl --context $PLATFORM apply --filename promise.yaml
```

As soon as the Promise is applied, Kratix will trigger an update for the existing application you deployed on the previous step. Once the pipeline completes, you should see a new Bucket on your MinIO server:
...
Excellent! You now have a fully automated pipeline that creates a new bucket for each application deployed on your Platform cluster.

However, there is a problem with this approach. The `create-bucket` step is not idempotent. If you run the pipeline again, you will get an error:
...
To make it idempotent, you need to store and retrieve the terraform state from the previous run -- if it exists. Hop on to the next section to learn how to do that.
...
To store the terraform state in a ConfigMap, the first step is to give the Kratix Pipeline Service Account the ability to create and retrieve ConfigMaps. The Service Account Kratix creates for the Resource Pipeline follows the format `<promise name>-resource-configure`.
...
kubectl --context $PLATFORM create clusterrolebinding promise-configmap \
    ...
    --serviceaccount=default:app-resource-configure
```
...
Since you didn't actually change the Promise itself, but rather the underlying Pipeline image, there's no need to re-apply the Promise. All you need to do is trigger a reconciliation of the existing resource. To do that, Kratix listens to a special label called `kratix.io/manual-reconciliation`. If you set that label to `true`, Kratix will trigger a reconciliation of that resource. You can do that with the following command:

```bash
# make sure the bucket no longer exist
mc rb kind/todo.default
...
Once the pipeline completes, you should see the ConfigMap for the todo app:
...
You may have noticed that, at this stage, the bucket the Pipeline is created won't be removed when the user deletes their App request. As an extra challange, you can try to implement a delete lifecycle hook for your resource. Take a look at the [Workflow reference docs](/main/reference/workflows) to find out more (tip: check the `workflows.resource.delete` property).
...
👉🏾&nbsp;&nbsp; Next, let's [explore how to surface informatino to users](./status).
