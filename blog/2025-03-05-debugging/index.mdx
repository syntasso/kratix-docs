...
The Kratix Marketplace contains a Runtime Promise which deploys a Deployment configured with Nginx. By making updates to the Promise and Kratix resources, we'll highlight some common problems and the breadcrumbs you can follow to solve them.
...
The `lifecycle` field determines which Destination the schedule the workloads to adn this maps to the label `environment=${lifecycle}` on the Destinations. Whilst working on the Promise, we want to deploy it to a `testing` Destination so the request will look as follows:
...
Lets apply this request with `kubectl apply -f`.

We can query for the pods created as part of the workflow with selectors, this is particularly useful in busy environments with a lot of running pods:
...
As the workflow has finished running, we can check to ensure the documents were scheduled to the `testing` Destination. Run the following to get the Work associated with the `example-runtime` resource:

```bash
kubectl get work --selector kratix.io/resource-name=example-runtime -o yaml
```

The status of the Work shows that it has not been scheduled with the following conditions:
...
What does this mean? Essentially, there were no Destinations matching the labels `environment=testing` in our environment. Lets review the available destinations and their labels:
...
Similarly, when querying the BucketStateStore
...
```bash
kubectl describe BucketStateStore testing --context kind-platform
```

The `status` of the BucketStateStore includes some conditions which details why it is not yet ready:
...
When creating both Destinations and State Stores, Kratix checks to see that the defined locations can be written to with the provided credentials before marking them as `Ready`. We see similar a similar `status` and `event` fired for the `testing` BucketStateStore:
...
Now that it's deployed successfully, we're now ready to build on the Runtime Promise and add the security scan as a new step when configuring resource requests.

We can bootstrap this step with the [Kratix CLI's](/main/kratix-cli/intro) `add container` command. From the root of the Promise directory, you can run:
...
```
NAME                      STATUS    AGE
runtime-example-runtime   healthy   8m12s
```
