...
What does this mean? Essentially, there were no Destinations matching the label `environment=testing` in our environment. Lets review the available Destinations and their labels:
...
The `status` of the BucketStateStore includes some conditions which detail why it is not yet ready:
