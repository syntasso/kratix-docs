...
# authors: [chunyi and chatgpt of course]
tags: [kratix, healthcheck, upgrade]
---
...
This container outputs [HealthDefinition](), which contains structured instructions on how to perform health checks in the Destination.
Kratix reads the health status and automatically updates the status of each resource request.

By including a healthCheck definition, you give Kratix insight into the runtime state of each resource, whether it’s ready, degraded, or failed.
...
Let’s say your Promise provisions PostgreSQL as a service You’re releasing a new version of Promise that updates some of it default configurations.
...
1. Kratix continues upgrading Resources one by one until all of them are ugpraded and reported healthy

This drastically reduces risk and improves debuggability during updates.

## Conclusion
If you’re interested in implementing health checks for your Promises, check out our a [step-by-step guide]().
We are keen to hear how you plan to use health checks and what other upgrade strategies you are interested in.

As always, feel free to drop by our Slack or GitHub for any questions and feedback.
