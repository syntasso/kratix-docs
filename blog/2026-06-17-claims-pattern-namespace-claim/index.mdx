...
You may decide _not to_ delete the resource when it has no claims. To do that, just don't include the delete workflow!
...
The configure and delete workflows both run the same container image from the Marketplace repo. If you want to adapt the pattern to another shared resource, copy that pipeline and swap what gets created and destroyed; the counting logic stays the same. Check out the [source code here](https://github.com/syntasso/kratix-marketplace/blob/main/namespace-claim/internal/configure-pipeline/claim).

</details>

## Try it

If you want to poke at it yourself, try out the promise available in the marketplace. Install the Promise, apply the examples below, and observe what the promise produces. Then delete them in order and watch what happens to the namespace on the worker:
