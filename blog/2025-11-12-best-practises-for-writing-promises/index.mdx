...
In this blog post, we’re going design a promise with some core fundamentals in mind, paving the way for better debug-ability,
...
We’ll start by creating a new directory for our Promise and  will use the [kratix cli](/main/kratix-cli/intro) to bootstrap it. Run:
...
```bash
kubectle describe redis example-redis
```
