...
if [ "$database_driver" = "postgresql" ]; then
  ...
  secret_ref="${app_name}.${appName}-bestdb-postgresql.credentials.postgresql.acid.zalan.do"

  # Inject PostgreSQL environment variables directly into runtime-request.yaml
  cat <<EOF >> /kratix/output/runtime-request.yaml
    - name: PGHOST
      value: ${app_name}-bestdb-postgresql.default.svc.cluster.local
    - name: DBNAME
      value: bestdb
    - name: PGUSER
      valueFrom:
...
if [ "$cache_driver" = "redis" ]; then
  ...
  cat <<EOF >> /kratix/output/runtime-request.yaml
    - name: REDIS_URL
      value: ${app_name}.default.svc.cluster.local
EOF
fi
```
...
Where do Compound Promises fit in this scenario? In the workflow of a Compound Promise, we will often create requests to the sub-promises, like we did in the pipeline script above. Those requests must be scheduled, then, to a Destination that can understand the CRD being used in the document. Since that CRD is itself installed when a Promise is installed, this setup often means the outputs of a Compound Promise workflow should be scheduled to the Platform cluster itself.

TODO: Add diagram

That means we need to tell Kratix to schedule the documents generated by the RubyApp Promise workflow to the Platform cluster itself. That can be done by setting Destination Selectors in the Promise. Open your Promise file and, under `spec`, add the following:
...
With the above declaration, we are telling Kratix _schedule the outputs of this Promise to a Destination with the label `cluster=platform`_. Therefore, we need to ensure that (1) there is a Destination with that label and (2) the Platform cluster itself is reconciling on the documents scheduled to that Destination.

We won't go into details on how to do that in this blog post. You can find more information on how to [Registering the Platform as a Destination](/workshop/installing-kratix#register-the-platform-as-a-destination) in the Kratix workshop.

// TODO detail setup of the platform as a destination with the above label

:::tip

To quickly get an environment compatible with the promises in this blog post,
clone Kratix and run
...
The final piece missing in our Compound Promise is the declaration of the sub-Promises it depends on. To define that, you set the `spec.requiredPromises` field in the Compount Promise document. It takes a list of sub-Promise names and versions.
